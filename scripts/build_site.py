...
CONCEPT_RUNS: dict[str, dict[str, str]] = {
    "optimal-control-problem": {
        "run": "Suppose a landing rocket is 80 meters above the pad, falling at 18 m/s, with 12 seconds of fuel left. Use a rough two-step horizon with dt = 1 second, state x = (height, velocity, fuel), and control u = upward acceleration from thrust. Gravity adds -10 m/s^2, and fuel drops by 2 units for each 1 m/s^2 of thrust. Candidate A burns hard now: u_0 = 18, then u_1 = 8. After the first second, velocity changes from -18 to -10 m/s and height falls to 70 meters; fuel drops from 12 to -24, so the path is illegal before it can be attractive. Candidate B burns less: u_0 = 4, then u_1 = 2. After two seconds, velocity is -32 m/s, height is 24 meters, and fuel is 0; it is legal on fuel but headed for a hard crash. Candidate C uses u_0 = 3 and u_1 = 3. It also keeps fuel nonnegative, ending with fuel 0, but it reaches only h_2 = 23 meters with v_2 = -32 m/s. The score may rank B and C, but A is rejected before scoring. These ugly numbers are the point: an optimal-control problem is not a wish to land softly. It is the named chain of states, controls, dynamics, cost, horizon, and constraints that lets the controller discover whether a soft landing is even available.",
        "math": "One finite-horizon form is: choose u_0...u_{N-1} to minimize J = sum_k fuel_cost(u_k) + 100*landing_error^2 + 20*touchdown_speed^2, subject to x_{k+1}=f(x_k,u_k), fuel_k >= 0, 0 <= u_k <= u_max, and safe touchdown speed. The rocket state update might be v_{k+1}=v_k + dt*(u_k - 10), h_{k+1}=h_k + dt*v_{k+1}, and fuel_{k+1}=fuel_k - 2*u_k. For Candidate B, the fuel constraint passes because 12 - 2*4 - 2*2 = 0, but a two-step terminal score with h_target = 0 and v_target = 0 gives 100*(24 - 0)^2 + 20*(-32)^2 = 78,080 before adding fuel cost. Candidate C has 12 - 2*3 - 2*3 = 0 and terminal score 100*(23 - 0)^2 + 20*(-32)^2 = 73,380. This one line ties the pieces together: the action changes acceleration, acceleration changes velocity, velocity changes height, fuel limits future actions, and the objective scores the whole resulting trajectory. If the fuel model is wrong or the constraint is omitted, the optimizer can produce a mathematically neat answer to the wrong landing problem.",
    },
    "state": {
