...
def main() -> int:
    ...
    for concept in concepts:
        ...
        if concept.get("id") == "optimal-control-problem":
            for marker in ["80 meters above the pad", "18 m/s", "12 seconds of fuel", "dt = 1 second", "x = (height, velocity, fuel)", "u_0 = 18", "u_1 = 8", "velocity changes from -18 to -10 m/s", "height falls to 70 meters", "fuel drops from 12 to -24", "u_0 = 4", "u_1 = 2", "velocity is -32 m/s", "height is 24 meters", "fuel is 0", "legal on fuel but headed for a hard crash", "u_0 = 3", "u_1 = 3", "h_2 = 23 meters", "v_2 = -32 m/s", "A is rejected before scoring", "2*u_0 + 2*u_1 &lt;= 12", "u_0 + u_1 &lt;= 6", "gravity subtracts 20 m/s", "v_2 = -18 + 6 - 20 = -32 m/s", "not inside this written problem", "Give the rocket 60 fuel units", "four-second horizon", "[8, 8, 8, 8]", "2*(8+8+8+8)=64 fuel units", "still illegal", "[7, 7, 7, 7]", "use 56 fuel units", "change velocity by 28 - 40 = -12 m/s", "v_4 = -30 m/s", "More fuel alone did not solve", "6 miles from a grocery store", "ice cream at -14 C", "arrival within 12 minutes", "cargo temperature at or below -10 C", "every 3 minutes", "Fast driving covers 2 miles per step", "slow driving covers 1 mile per step", "High cooling uses 4 battery units", "low cooling uses 1 battery unit", "truck has 10 battery units", "Plan A drives fast for 3 steps with high cooling", "battery use is 12 units", "the plan is illegal", "Plan B drives fast for 3 steps with low cooling", "temperature rises by 0.5 C each step", "end at -12.5 C", "Plan C drives slow for 4 steps with high cooling", "uses 16 battery units", "misses the store by 2 miles", "distance update, time count, temperature update, and battery limit agree", "choose u_0...u_{N-1}", "J = sum_k fuel_cost(u_k) + 100*landing_error^2 + 20*touchdown_speed^2", "x_{k+1}=f(x_k,u_k)", "fuel_k &gt;= 0", "v_{k+1}=v_k + dt*(u_k - 10)", "h_{k+1}=h_k + dt*v_{k+1}", "fuel_{k+1}=fuel_k - 2*u_k", "12 - 2*4 - 2*2 = 0", "100*(24 - 0)^2 + 20*(-32)^2 = 78,080", "12 - 2*3 - 2*3 = 0", "100*(23 - 0)^2 + 20*(-32)^2 = 73,380", "feasibility check is separate from the ranking check", "sum_u = u_0 + u_1 &lt;= 6", "v_2 = -18 + sum_u - 20", "every legal sequence has v_2 &lt;= -32", "|v_2| &lt;= 2", "feasible set is empty", "fuel_0=60", "2*sum_{k=0}^3 u_k &lt;= 60", "sum_u &lt;= 30", "v_4 = -18 + sum_u - 40", "v_4 &lt;= -28", "|v_4| &lt;= 2", "sum_u = 56", "v_4 = -18 + 56 - 40 = -2", "fuel use would be 112 units", "problem pieces must agree", "not to tune weights harder", "change thrust limits"]:
                if marker not in text:
                    errors.append(f"optimal control problem page missing trajectory-chain marker: {marker}")
            if len(words) < 1580:
                errors.append(f"optimal control problem page below core richness floor: {len(words)} < 1580")
        if concept.get("id") == "action-control-input":
            for marker in ["h = 10 meters", "v = 0.2 m/s", "48 percent above hover", "dt = 0.1 seconds", "a(u)=4.8 m/s^2", "v_next = 0.2 + 0.1*4.8 = 0.68 m/s", "h_next = 10 + 0.1*0.68 = 10.068 meters", "6.8 centimeters", "u = 0.9, meaning 90 percent above hover", "u_sent = 0.5", "0.2 + 0.1*5.0 = 0.70 m/s", "next height is 10.070 meters", "u_prev = 0.10", "change thrust by only 0.20 per tick", "u_rate = 0.30", "a(u_rate)=3.0 m/s^2", "v_next = 0.2 + 0.1*3.0 = 0.50 m/s", "h_next = 10 + 0.1*0.50 = 10.050 meters", "battery_temp = 46 C", "safe set may shrink to u &lt;= 0.25", "u_hot = 0.25", "a(u_hot)=2.5 m/s^2", "next height is only 10.045 meters", "healthy battery", "u = 0.30 gives a(u)=3.0 m/s^2", "low voltage", "same requested u = 0.30", "a_low(u)=1.8 m/s^2", "v_next = 0.2 + 0.1*1.8 = 0.38 m/s", "h_next = 10.038 meters", "Same command, different physical response", "battery_voltage", "planner expects height 10.050", "measures 10.038", "0.012 meter shortfall", "north, south, east, west is discrete", "u = 0.37 is continuous", "limits, rate changes, motors, acceleration, velocity, and only then position", "a(u)=10*u", "-0.3 &lt;= u &lt;= 0.5", "v_next = v + dt * a(u)", "h_next = h + dt * v_next", "u_clipped = min(0.9,0.5) = 0.5", "a=5.0 m/s^2", "v_next = 0.2 + 0.1*5.0 = 0.70", "h_next = 10 + 0.1*0.70 = 10.070", "|u - u_prev| &lt;= 0.20", "u_rate = 0.30 for this tick", "a(u_rate)=3.0", "v_next = 0.2 + 0.1*3.0 = 0.50", "h_next = 10 + 0.1*0.50 = 10.050", "U(x)", "U(x_hot) = [-0.3, 0.25]", "u_safe = min(0.48,0.25) = 0.25", "h_next = 10 + 0.1*(0.2 + 0.1*2.5) = 10.045", "command-to-effect map can also depend on state", "For x_normal, a(x,u)=10*u", "For x_low_voltage, a(x,u)=6*u", "same u=0.30 gives a=1.8 and h_next=10.038", "10.050 - 10.038 = 0.012 meters", "After ten such ticks", "about 0.12 meters", "if |u| &lt; 0.05, then a(u)=0", "u = 0.03", "h_next = 10.020", "x_next = f(x, u_rate)", "command that actually reaches the plant", "(h_next - 12)^2", "actuator truth", "delay, saturation, dead zones, state-dependent safe sets, voltage-dependent response, and rate limits"]:
                if marker not in text:
                    errors.append(f"action concept page missing command-vs-outcome marker: {marker}")
            if len(words) < 1520:
                errors.append(f"action concept page below core richness floor: {len(words)} < 1520")
        ...
        if concept.get("id") == "objective-cost-function":
            for marker in ["Path A reaches the spot in 8 seconds", "Path B takes 12 seconds", "8 is smaller than 12", "steering effort 10", "wall risk 30", "steering effort 3", "Path A: 8 + 0.5*10 + 1.0*30 = 43", "Path B: 12 + 0.5*3 + 1.0*0 = 13.5", "wall weight 0", "Path A scores 13 and beats 13.5", "clearance must be at least 10 centimeters", "Path A is illegal because 6 &lt; 10", "soft wall penalty trades wall risk against time", "hard wall constraint refuses the trade", "Path C reaches in 7 seconds", "paint_damage = 1", "cost omits damage", "C scores 7 + 0.5*4 = 9", "repair cost adds 20*paint_damage", "C scores 7 + 0.5*4 + 20*1 = 29", "contact is forbidden", "scratch was written as a cost term or constraint", "five bumper brushes have written damage cost 0", "real shop sees five repairs", "thermostat from the course overview", "room target is 70 F", "temperatures 69, 70, 71", "uses 3 heater units", "Plan B keeps 67, 68, 70", "uses 1 heater unit", "temperature_error^2 + 0.2*heat", "total 2.6", "total 13.2", "energy weight rises to 5.0", "Plan A adds 15 and scores 17", "Plan B adds 5 and scores 18", "terminal cost pays 20*(final_error)^2", "B wins despite worse early tracking", "68 &lt;= temp &lt;= 72", "Plan B starts at 67 F", "rejected before scoring", "written measurement of what counts as better", "J = sum_k stage_cost(x_k,u_k) + terminal_cost(x_N)", "w_wall*wall_risk + w_damage*paint_damage", "stage_0 + stage_1 + stage_2", "0.5*steering_effort", "A has base score 8 + 0.5*10 = 13", "B has 12 + 0.5*3 = 13.5", "30*w_wall", "13 + 30*w_wall &lt; 13.5", "w_wall &lt; 0.5/30 = 0.0167", "Any wall weight above 0.0167 flips the written choice to B", "clearance_A = 0.06 meters", "clearance_min = 0.10 meters", "clearance_A - clearance_min = -0.04 meters", "A is infeasible before J is compared", "With w_damage=0", "J_C=7 + 0.5*4 + 0*1 = 9", "With w_damage=20", "J_C=7 + 0.5*4 + 20*1 = 29", "9 + w_damage &lt; 13.5", "w_damage &lt; 4.5", "same maneuver repeats five times", "5*0 = 0", "5*20 = 100", "physical bill each zero in the cost is hiding", "real repair bill is 200 dollars", "written exchange rate says only 4 points", "stage_cost_k=(temp_k - 70)^2 + alpha*heat_k", "terminal_cost=beta*(temp_3 - 70)^2", "alpha=0.2 and beta=0", "J_A=(1+0+1)+0.2*3=2.6", "J_B=(9+4+0)+0.2*1=13.2", "alpha=5 and beta=0", "J_A=2+15=17", "J_B=13+5=18", "alpha=5 and beta=20", "J_A=17+20*(1^2)=37", "J_B=18+20*(0^2)=18", "67 - 68 = -1", "B is infeasible even though its terminal value is good", "written exchange rates and written constraints changed", "follows the score and the feasibility rules", "proxy honesty"]:
                if marker not in text:
                    errors.append(f"objective concept page missing tradeoff marker: {marker}")
            if len(words) < 1550:
                errors.append(f"objective concept page below core richness floor: {len(words)} < 1550")
        if concept.get("id") == "horizon":
            for marker in ["60 meters from the pad", "18 percent battery", "full-speed flight uses 2 percent battery", "slow flight uses 1 percent battery", "A three-second horizon sees only the next 24 meters", "crosswind band from 25 meters to 40 meters", "at least 14 percent battery", "five-second horizon", "distance_5 = 5*8 = 40 meters", "battery_5 = 18 - 5*2 = 8 percent", "violates the 14 percent reserve rule", "ten-second horizon", "Full speed would travel 80 meters", "overshooting the 60 meter pad by 20 meters", "slow speed would be only 40 meters", "neither simple plan is acceptable", "shaped speed plan", "charging stop, or a different route", "120-second horizon", "guessed storm cell after landing", "50 percent battery penalty", "not useful for the first 5 seconds of flight", "weak far-future story", "price a story instead of the flight", "hallway cart", "9 meters from a closed door", "moves at 2 m/s", "brake at at most 1 m/s^2", "radio signal opens the door only after 4 seconds", "distance_2 = 2*2 = 4 meters", "door event time is t_door = 9/2 = 4.5 seconds", "five-second horizon reaches the door", "waits 3 seconds before braking", "v^2/(2a)=2^2/(2*1)=2 meters", "ending at p = 8 meters with 1 meter of margin", "waits 4 seconds", "would hit the door at 9 meters before stopping", "forty-second horizon may include a guessed hallway crowd", "first brake-or-wait choice", "distance_3 = 3*8 = 24 meters", "battery_3 = 18 - 3*2 = 12 percent", "distance_3 = 3*4 = 12 meters", "battery_3 = 18 - 3*1 = 15 percent", "At 5 seconds", "distance_5 = 40 meters", "battery_5 = 8 percent", "battery_at_wind = 8 &lt; 14", "At 10 seconds", "distance_10 = 10*8 = 80 meters", "overshoot_10 = 80 - 60 = 20 meters", "distance_10 = 10*4 = 40 meters", "battery_10 = 18 - 10*1 = 8 percent", "battery_at_wind &gt;= 14 percent", "dt = 0.5 seconds", "N = 60", "t_event = 25/8 = 3.125 seconds", "3 &lt; 3.125", "5 &gt;= 3.125", "band exit at 40/8 = 5 seconds", "added model is trusted", "storm_penalty = 0.5*50 = 25 expected battery points", "swamp the near wind reserve", "Too short is blind; too long can be expensive or misleading", "first consequence time", "first action still leaves a legal future"]:
                if marker not in text:
                    errors.append(f"horizon concept page missing delayed-consequence marker: {marker}")
            if len(words) < 1520:
                errors.append(f"horizon concept page below core richness floor: {len(words)} < 1520")
        if concept.get("id") == "constraints":
            for marker in ["4 centimeters from the shelf", "12 newton-meters", "t = 0.4 seconds", "3 centimeters from the shelf", "t = 0.6 seconds", "15 newton-meters", "5 centimeters", "11 newton-meters", "3 &lt; 4 and 15 &gt; 12", "2 points per missing centimeter", "Candidate C uses only 4 energy points", "2 centimeters from the shelf", "4 + 2*(4 - 2) = 8", "8 &lt; 11", "Candidate C must still be rejected", "wrist would like torque 14", "minimizes (tau - 14)^2", "tau = 14 illegal", "best legal torque is tau = 12", "upper torque constraint is active", "tau = 10", "2 newton-meters of slack", "From tau = 12", "move upward", "leaves the feasible interval", "both small upward and downward changes are still legal", "Constraints also have addresses in time", "Candidate D stays 5 centimeters", "never asks for more than 10 newton-meters", "camera 6 centimeters away", "final_pose_error &lt;= 1 centimeter", "middle path limits passed", "clean endpoint cannot forgive an illegal middle", "clean middle cannot forgive a missed endpoint", "initial_clearance = 2 centimeters", "initial_clearance &gt;= 4 centimeters", "illegal before the first command", "Candidate F has nominal clearance 5 centimeters", "passes the written 4 centimeter shelf rule by 1 centimeter", "shelf can flex inward by 0.8 centimeters", "pose estimate can be wrong by 0.7 centimeters", "5 - 0.8 - 0.7 = 3.5 centimeters", "real check fails", "nominal clearance &gt;= 5.5 centimeters", "1.5 centimeters of uncertainty margin", "cheap penalty", "buy collision risk with energy savings during normal factory operation", "g(x,u) &lt;= 0", "g_clear,k = 0.04 - distance_to_shelf(x_k) &lt;= 0", "distance_to_shelf = 0.03", "g_clear = 0.04 - 0.03 = 0.01 &gt; 0", "g_tau,k = abs(tau_k) - 12 &lt;= 0", "g_tau = 15 - 12 = 3 &gt; 0", "g_clear = -0.01", "g_tau = -1", "Candidate C has distance 0.02", "g_clear = 0.04 - 0.02 = 0.02 &gt; 0", "rho*max(0,g_clear)", "100*0.02 = 2", "not the same statement as g_clear &lt;= 0", "J(tau)=(tau - 14)^2", "-12 &lt;= tau &lt;= 12", "unconstrained minimizer is tau = 14", "violates tau &lt;= 12", "g_upper(tau)=tau - 12 = 0", "upper bound is active", "g_upper(tau)=10 - 12 = -2", "bound is inactive", "defect_k = x_{k+1} - f(x_k,u_k) = 0", "h_0(x_0)=0", "g_k(x_k,u_k) &lt;= 0 for every k", "h_N(x_N)=0", "pose_error_N &lt;= 0.01 meters", "pose_error_N = 0.06 meters", "pose_error_N - 0.01 = 0.05 &gt; 0", "0.04 - initial_clearance = 0.02 &gt; 0", "worst_clearance = nominal_clearance - shelf_flex - pose_error_bound", "0.05 - 0.008 - 0.007 = 0.035 meters", "0.04 - 0.035 = 0.005 &gt; 0", "distance_to_shelf &gt;= 0.055", "0.04 + 0.008 + 0.007 = 0.055", "100*0.005 = 0.5 points", "one-point energy saving can still make the unsafe path win", "real forbidden event", "time when it can happen", "uncertainty that can move a passing nominal number across the line", "shelf flex, cable snag, heat, or human clearance"]:
                if marker not in text:
                    errors.append(f"constraints concept page missing hard-limit marker: {marker}")
            if len(words) < 1550:
                errors.append(f"constraints concept page below core richness floor: {len(words)} < 1550")
        ...
        if concept.get("id") == "collocation":
            for marker in ["x = 0.02 meters", "x = 0.18 meters", "t = 0.2 seconds", "x = 0.10 meters", "x = 0.08 to 0.12 meters", "path_derivative_mid = 0.9 m/s", "f(x_mid,u_mid)=0.4 m/s", "curve is trying to pass through", "x_mid = 0.14 meters", "outside the fixture", "f(x_mid,u_mid)=0.6 m/s", "curve slope is 0.6 m/s", "passes both checks", "one honest midpoint is not a free pass", "t = 0.3 seconds", "x_3 = 0.16 meters", "clearance_3 = 0.16 - 0.12 = 0.04 meters", "defect_3 = 0.5 - 0.5 = 0", "x_3 = 0.11 meters", "fixture collision has merely moved later", "refine the mesh between two passing checks", "x_2 = 0.14 at t = 0.2", "x_3 = 0.16 at t = 0.3", "cubic curve bows inward to x_25 = 0.115 at t = 0.25", "0.115 - 0.12 = -0.005 meters", "coarse mesh missed a real collision", "Adding t = 0.25 as a collocation or constraint point", "hidden dip", "A mesh can also hide a short actuator burst", "u_2 = 3 Nm and u_3 = 3 Nm", "under a 5 Nm limit", "u_25 = 7 Nm", "has not earned that shove", "refined constraint point makes the burst visible", "must change at least one coefficient", "stops dipping through the fixture", "changes the questions the candidate must answer", "Between t = 0.2 and t = 0.4", "dt = 0.2 seconds", "x_2 = 0.14 and x_4 = 0.18", "x_4 - x_2 = 0.04 meters", "0.2*(0.6 + 0.2)/2 = 0.08 meters", "interval defect is 0.04 - 0.08 = -0.04 meters", "Changing x_4 to 0.22", "wall limit x &lt;= 0.20", "clearance, dynamics, actuator bounds, and endpoint goals", "selected interior points and intervals to be physically honest", "defect_mid = path_derivative_mid - f(x_mid,u_mid)", "defect_mid = 0.9 - 0.4 = 0.5 m/s", "driving defect_mid to 0", "clearance_mid &gt;= 0", "x_mid = 0.10 meters", "clearance_mid is negative", "clearance_mid = 0.14 - 0.12 = 0.02 meters", "defect_mid = 0.6 - 0.6 = 0", "clearance_3 = x_3 - 0.12", "defect_3 = path_derivative_3 - f(x_3,u_3)", "clearance_3 = 0.04", "clearance_3 = -0.01", "candidate fails even if the midpoint looked repaired", "refined mesh asks whether the polynomial segment", "x_25 = 0.115 at t = 0.25", "clearance_25 = x_25 - 0.12 = -0.005 meters", "same curve fails after adding the new check", "defect_interval = (x_{k+1} - x_k) - dt*(f_k + f_{k+1})/2", "defect_interval = (0.18 - 0.14) - 0.2*(0.6 + 0.2)/2 = -0.04 meters", "Setting x_4=0.22", "(0.22 - 0.14) - 0.08 = 0", "x_4 &lt;= 0.20 rejects it", "move the right amount over each checked interval", "t = 0.1, 0.2, 0.25, and 0.3 seconds", "sampling and representation", "unmodeled flex, backlash, or contact event"]:
                if marker not in text:
                    errors.append(f"collocation concept page missing midpoint marker: {marker}")
            if len(words) < 1520:
                errors.append(f"collocation concept page below core richness floor: {len(words)} < 1520")
        if concept.get("id") == "lqr":
            for marker in ["20 centimeters left", "e_next = e + u", "u = -0.190 meters", "e = 0.05", "u = -(20/21)*0.05 = -0.0476 meters", "e = -0.20", "u = +0.190 meters", "measure the deviation, multiply by a gain", "4*u^2", "u = -0.1667 meters", "e_next = 0.0333 meters", "0.25*u^2", "u = -0.1975 meters", "e_next = 0.0025 meters", "x = [lane_error, heading_error] = [0.20, 0.10]", "u = -0.8*lane_error - 0.4*heading_error", "u = -0.8*0.20 - 0.4*0.10 = -0.20 meters", "x = [0.00, 0.10]", "u = -0.04 meters", "before lane error appears", "70 centimeters left", "u = -(20/21)*0.70 = -0.667 meters", "|u| &lt;= 0.10", "e_next = 0.60 meters", "60 percent of the expected sideways change", "e_measured = 0.70 + 0.6*(-0.10) = 0.64 meters", "not a small miss around the center line", "needs a wider recovery plan", "spends actuator effort now or tolerates more remaining error", "x_{k+1}=A_k x_k + B_k u_k", "J(u)=u^2 + 20*(0.20 + u)^2", "2u + 40*(e + u)=0", "u = -(20/21)*e", "K = 20/21", "R = 4", "4u^2 + 20*(e+u)^2", "8u + 40*(e+u)=0", "u = -(5/6)*e", "K = 5/6", "R = 0.25", "0.5u + 40*(e+u)=0", "u = -(40/40.5)*e", "u_k = -K_k x_k", "K = [0.8, 0.4]", "u = -[0.8,0.4]*[0.20,0.10] = -0.20", "same gain on x = [0.00,0.10] gives u = -0.04", "|u| &lt;= 0.10 meters", "plain LQR asks for u = -0.190", "actuator can only deliver u = -0.10", "e_next = 0.10", "not the predicted 0.010", "|e_next| &lt;= 0.05", "0.10 &gt; 0.05", "For e = 0.70", "unconstrained feedback asks for u = -0.667", "e_next = e + 0.6*u", "0.70 + 0.6*(-0.10) = 0.64 meters", "linearization around the center line", "tire did not deliver", "actuator saturates"]:
                if marker not in text:
                    errors.append(f"LQR concept page missing feedback-gain marker: {marker}")
            if len(words) < 1450:
                errors.append(f"LQR concept page below core richness floor: {len(words)} < 1450")
        ...
        if concept.get("id") == "behavioral-cloning":
            for marker in ["1,000 centered-lane frames", "steering angle -12 degrees", "steering angle +12 degrees", "average label is 0 degrees", "1 degree right bias over 20 frames", "0.25 meters toward the lane edge", "expert would steer -8 degrees left", "200 such frames", "37 off-center frames", "add those 37 pairs to the dataset", "20 of the 1,000 centered-lane frames", "human was distracted", "steering +6 degrees right", "(980*0 + 20*6)/1000 = 0.12 degrees right", "over 100 control ticks", "drift 0.30 meters", "Bad labels are not warnings to the learner; they are targets", "poor demonstrations put poor behavior directly into the policy", "good test error on expert states can hide bad closed-loop recovery", "L(theta)=sum_i ||pi_theta(x_i)-u_i^expert||^2", "50*(a - (-12))^2 + 50*(a - 12)^2", "squared-error optimum is a = 0 degrees", "980*a^2 + 20*(a - 6)^2", "1960a + 40*(a - 6) = 2000a - 240", "a = 0.12 degrees", "0.12 degree bias moves lateral position by 0.003 meters", "100*0.003 = 0.30 meters", "d_expert(x)", "d_pi_theta(x)", "x_{t+1}=f(x_t,pi_theta(x_t))", "37 off-center frames out of 200", "D_1 = D_0 union {(x_j^learner,u_j^expert)}", "no built-in exploration or reward signal", "no built-in way to know that a labeled expert action was careless", "Filtering labels, weighting trusted demonstrations, or adding recovery data"]:
                if marker not in text:
                    errors.append(f"behavioral cloning concept page missing supervised-action marker: {marker}")
            if len(words) < 1380:
                errors.append(f"behavioral cloning concept page below core richness floor: {len(words)} < 1380")
        ...
        if concept.get("id") == "hamiltonian-optimal-control":
            for marker in ["running cost is 0.5*u^2", "x_dot = u", "p = -20", "H = 0.5*u^2 + p*u", "u = 0: H = 0", "u = 10: H = 50 - 200 = -150", "u = 20: H = 200 - 400 = -200", "u = 30: H = 450 - 600 = -150", "motor cap u &lt;= 12", "H(12)=0.5*12^2 - 20*12 = 72 - 240 = -168", "illegal H(20)=-200", "legal H(10)=-150", "constrained local choice sits at u = 12", "p = -4", "H(u)=0.5*u^2 - 4*u", "unconstrained balance is u = 4", "H(4)=8-16=-8", "H(12)=72-48=24", "cap is legal but no longer attractive", "p = +6", "every positive thrust raises the local account", "best local choice is u = 0", "same motor and the same running cost", "Only the backward price changed", "overheats above 8 units of thrust", "3*(u - 8)^2", "hidden heat bill is 3*4^2 = 48", "old capped score -168 becomes -120", "0.5*8^2 - 20*8 = 32 - 160 = -128", "safer thrust now wins", "written account was missing a real bill", "H(x,u,p)=L(x,u)+p*f(x,u)", "H(u)=0.5*u^2 - 20*u", "dH/du = u - 20 = 0", "dH/du at u=12 = 12 - 20 = -8", "increasing u is forbidden", "active boundary u^* = 12", "dH/du = u - 4", "stationarity gives u=4", "0 &lt;= 4 &lt;= 12", "cap is inactive", "dH/du = u + 6", "minimum over 0 &lt;= u &lt;= 12 is the lower boundary u=0", "one-sided check", "admissible directions only allow du &lt;= 0", "H(11)=60.5 - 220 = -159.5", "dH/du = u - 20 + 6*(u - 8)", "12 - 20 + 24 = 16", "H_heat(12)=-120", "H_heat(8)=-128", "terminal penalty can make that concrete", "p=-20 to p=-5", "stationary thrust moves from u=20 to u=5", "local in time", "costate came from the rest of the trajectory", "target, obstacle, or terminal penalty changes", "augment the cost with the constraint", "omits motor heat", "derivative-zero answer can be illegal", "not proved the whole path is best"]:
                if marker not in text:
                    errors.append(f"Hamiltonian concept page missing local-accounting marker: {marker}")
            if len(words) < 1450:
                errors.append(f"Hamiltonian concept page below core richness floor: {len(words)} < 1450")
        ...
        if concept.get("id") == "value-based-rl":
            for marker in ["battery 22 percent", "Q(x,turn_left) = 3", "Q(x,turn_right) = 7", "gamma = 0.9", "y = -1 + 0.9*10 = 8", "alpha = 0.5", "7 + 0.5*(8 - 7) = 7.5", "Q(x_next,charge_now)=10", "Q(x_next,inspect_box)=4", "epsilon-greedy behavior policy", "slow charger line", "y_SARSA = -1 + 0.9*4 = 2.6", "7 + 0.5*(2.6 - 7) = 4.8", "y_Q = -1 + 0.9*10 = 8", "Same first transition, different target", "only two smooth-floor samples", "returns 11 and 9", "three slick-floor samples for charge_now", "returns -4, -2, and 0", "(11 + 9 - 4 - 2 + 0)/5 = 2.8", "y_repaired = -1 + 0.9*4 = 2.6", "-1 + 0.9*2.8 = 1.52", "greedy choice can flip without changing the reward formula", "V_pi(x)", "Q_pi(x,u)", "policy evaluation", "policy improvement", "Q(x,u) &lt;- Q(x,u) + alpha*(y - Q(x,u))", "max_a Q(x_next,a)=10", "behavior policy is the rule that collected the sample", "target policy is the rule used inside the update", "SARSA uses y_SARSA = r + gamma Q(x_next,u_next)", "Q-learning uses y_Q = r + gamma max_a Q(x_next,a)", "SARSA learns the value of behaving with exploration still present", "Q-learning learns toward the greedy policy", "A Q entry is an estimate, not a fact", "Two returns [11,9] give mean 10", "slick-floor returns [-4,-2,0] gives mean 2.8", "target changes from 8 to 2.6", "old Q(x,turn_right)=7 toward 2.6", "representation honesty and coverage", "lucky early samples", "slick floors"]:
                if marker not in text:
                    errors.append(f"value-based RL concept page missing value-update marker: {marker}")
            if len(words) < 1400:
                errors.append(f"value-based RL concept page below core richness floor: {len(words)} < 1400")
        if concept.get("id") == "policy-optimization":
            for marker in ["body leaning forward 6 degrees", "right knee bent 20 degrees", "pi_theta(long_step|x) = 0.30", "pi_theta(short_step|x) = 0.70", "return G = 12", "return G = 2", "baseline b(x)=7", "A_long = 12 - 7 = +5", "A_short = 2 - 7 = -5", "pi_theta_new(long_step|x) = 0.38", "probability shift is earned by advantage", "old expected return from this two-action choice is 0.30*12 + 0.70*2 = 5.0", "After the small update it is 0.38*12 + 0.62*2 = 5.8", "floor material is missing from the state", "pi_theta(long_step|x) down to 0.05", "pi_theta(short_step|x) up to 0.95", "0.05*12 + 0.95*2 = 2.5", "state was too blunt", "floor=rubber and floor=tile", "keep pi(long_step|rubber) near 0.38", "dropping pi(long_step|tile) near 0.05", "J(theta)=E_{tau~pi_theta}[G(tau)]", "grad_theta log pi_theta(u_t|x_t) * A_t", "theta_new = theta + eta*g", "eta = 0.04", "Delta score_long = 0.04*5 = 0.20", "Delta score_short = 0.04*(-5) = -0.20", "surrogate objective", "tile floor", "return G = -8", "A_tile = -8 - 7 = -15", "eta = 0.20", "Delta score = 0.20*(-15) = -3.0", "rubber mat and tile floor are different states", "probability-ratio check", "old pi(long_step|x)=0.30 and new pi(long_step|x)=0.05", "0.05/0.30 = 0.167", "ratios from 0.8 to 1.2", "expected-return arithmetic", "5.8 to 0.05*12 + 0.95*2 = 2.5", "Splitting the state makes two local updates", "A_rubber=+5", "A_tile=-15", "erased a useful rubber-mat action", "Noisy returns"]:
                if marker not in text:
                    errors.append(f"policy optimization concept page missing direct-policy-update marker: {marker}")
            if len(words) < 1500:
                errors.append(f"policy optimization concept page below core richness floor: {len(words)} < 1500")
