...
def main() -> int:
    ...
    for concept in concepts:
        ...
        if concept.get("id") == "optimal-control-problem":
            for marker in ["80 meters above the pad", "18 m/s", "12 seconds of fuel", "dt = 1 second", "x = (height, velocity, fuel)", "u_0 = 18", "u_1 = 8", "velocity changes from -18 to -10 m/s", "height falls to 70 meters", "fuel drops from 12 to -24", "u_0 = 4", "u_1 = 2", "velocity is -32 m/s", "height is 24 meters", "fuel is 0", "legal on fuel but headed for a hard crash", "u_0 = 3", "u_1 = 3", "h_2 = 23 meters", "v_2 = -32 m/s", "A is rejected before scoring", "2*u_0 + 2*u_1 &lt;= 12", "u_0 + u_1 &lt;= 6", "gravity subtracts 20 m/s", "v_2 = -18 + 6 - 20 = -32 m/s", "not inside this written problem", "Give the rocket 60 fuel units", "four-second horizon", "[8, 8, 8, 8]", "2*(8+8+8+8)=64 fuel units", "still illegal", "[7, 7, 7, 7]", "use 56 fuel units", "change velocity by 28 - 40 = -12 m/s", "v_4 = -30 m/s", "More fuel alone did not solve", "choose u_0...u_{N-1}", "J = sum_k fuel_cost(u_k) + 100*landing_error^2 + 20*touchdown_speed^2", "x_{k+1}=f(x_k,u_k)", "fuel_k &gt;= 0", "v_{k+1}=v_k + dt*(u_k - 10)", "h_{k+1}=h_k + dt*v_{k+1}", "fuel_{k+1}=fuel_k - 2*u_k", "12 - 2*4 - 2*2 = 0", "100*(24 - 0)^2 + 20*(-32)^2 = 78,080", "12 - 2*3 - 2*3 = 0", "100*(23 - 0)^2 + 20*(-32)^2 = 73,380", "feasibility check is separate from the ranking check", "sum_u = u_0 + u_1 &lt;= 6", "v_2 = -18 + sum_u - 20", "every legal sequence has v_2 &lt;= -32", "|v_2| &lt;= 2", "feasible set is empty", "fuel_0=60", "2*sum_{k=0}^3 u_k &lt;= 60", "sum_u &lt;= 30", "v_4 = -18 + sum_u - 40", "v_4 &lt;= -28", "|v_4| &lt;= 2", "sum_u = 56", "v_4 = -18 + 56 - 40 = -2", "fuel use would be 112 units", "problem pieces must agree", "not to tune weights harder", "change thrust limits"]:
                if marker not in text:
                    errors.append(f"optimal control problem page missing trajectory-chain marker: {marker}")
            if len(words) < 1360:
                errors.append(f"optimal control problem page below core richness floor: {len(words)} < 1360")
