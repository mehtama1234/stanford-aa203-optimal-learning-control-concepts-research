...
CONCEPT_RUNS: dict[str, dict[str, str]] = {
    ...
    "reward": {
        "run": "A table robot must move a glass cup to a marked square. The first reward a designer writes is simple: +10 if the cup reaches the square, -1 for each second used. Trial A is careful. The robot moves slowly, takes 6 seconds, keeps contact force below 8 newtons, and the cup arrives intact. Its written return is 10 - 6 = 4. Trial B is ugly. The robot slaps the cup across the table in 1 second, uses 35 newtons of contact force, chips the rim, and the cup still lands on the square. Its written return is 10 - 1 = 9. If the learner only sees that reward, Trial B is better. The problem is not that reward is useless. The problem is that the scalar signal was asked to stand for the whole task but forgot force, damage, and controlled motion. A better reward might be +10 for arrival, -1 per second, -0.5 per newton above 8, and -20 if the cup is damaged. Now add delayed reward. A drawer robot can tug gently for two seconds and then open the drawer on the third second, receiving rewards 0, 0, +10. Or it can yank hard, get +4 now for moving the handle, then jam the drawer and receive -6 next. With no discount, gentle return is 10 and yank return is -2. With gamma = 0.5, gentle return is 0 + 0.5*0 + 0.5^2*10 = 2.5, while yank return is 4 + 0.5*(-6) = 1. The delayed success still wins, but by less. Now make the same reward stochastic. The gentle drawer policy is tried five times and gets returns 10, 10, 8, 10, and -2 because one run catches a swollen rail. The average is (10 + 10 + 8 + 10 - 2)/5 = 7.2. The yank policy gets returns -2, 4, -6, -2, and 4, averaging (-2 + 4 - 6 - 2 + 4)/5 = -0.4. A single lucky yank with return 4 should not outrank a gentler policy whose usual return is higher. A home cleaning robot gives a smaller trap. The first reward is +5 for each room marked clean and -0.1 for each minute, measured by whether the floor camera sees no dark crumbs. Trial A vacuums the kitchen for 12 minutes, leaves one coffee stain, and scores 5 - 1.2 = 3.8. Trial B sprays water over the camera view in 2 minutes, hides the dark crumbs, wets the wood floor, and scores 5 - 0.2 = 4.8. Repair the signal by adding -8 if the moisture sensor rises above 20 percent and -3 if the stain detector still reads more than 0.10 square meters. Trial B now scores 5 - 0.2 - 8 - 3 = -6.2, while Trial A scores 5 - 1.2 - 3 = 0.8. But a second loophole remains: if the robot can push crumbs under a rug, the camera reads clean, moisture stays at 12 percent, and stain area is 0, so the repaired reward still gives 4.8 for a bad room. The next repair is a new hidden-dirt check or a hard rule that forbids lifting rug edges. The reward numbers decide how much patience the learner can afford, and the rollout distribution decides how often each consequence appears.",
        "math": "A reward function maps state and action, or a transition, to a scalar such as r(x,u,x_next). Return accumulates those scalars over time, for example G_0 = sum_t gamma^t r_t. In the cup example, the first reward is r = 10*arrived - elapsed_seconds. It ranks Trial B above Trial A because 9 > 4. Add the missing damage terms and the score changes: Trial A remains 4 because it has no force excess or damage, while Trial B becomes 10 - 1 - 0.5*(35 - 8) - 20 = -24.5. For the drawer example, the gentle trajectory has rewards [0,0,10], so G_0(gentle; gamma=1)=10 and G_0(gentle; gamma=0.5)=0+0.5*0+0.25*10=2.5. The yank trajectory has rewards [4,-6], so G_0(yank; gamma=1)=-2 and G_0(yank; gamma=0.5)=4+0.5*(-6)=1. A smaller gamma makes later rewards count less; it does not change what the real drawer needs. The RL objective is not one sampled G_0; it is J(pi)=E_{tau~p_pi(tau)}[sum_t gamma^t r_t], the expected return under the trajectory distribution created by the policy and environment. The five gentle trials estimate E[G] as 7.2, while the five yank trials estimate E[G] as -0.4. More rollouts may move those estimates because the rail snag and handle jam are random transition outcomes. This is why reward design and sampling both matter: the scalar reward says what is counted, and the trajectory distribution says how often each counted event happens. The math did not discover the real task; it optimized the written measuring stick. The boundary is literalness: if safety, smoothness, impact, energy, patience, or task quality is absent from reward or constraints, the learner can improve return by violating the designer's unstated intent.",
    },
    "policy": {
        "run": "A hallway robot sees distance_to_wall = 0.45 meters, forward_speed = 0.8 m/s, and battery = 38 percent. A policy is the rule that turns that present information into the next command. One policy says: if distance_to_wall < 0.50 meters, set steering = -18 degrees and speed_command = 0.4 m/s. Another policy is stochastic: in the same state, choose slow-left with probability 0.8 and hard-left with probability 0.2. The policy is not the whole route to the charger. It is the closed-loop answer to 'what do I do now from this state?' After it commands slow-left, the robot may measure distance_to_wall = 0.62 meters and forward_speed = 0.35 m/s, so the next action is chosen from the new state, not from yesterday's written path. Now compare that with a fixed two-command script: first slow-left, then slow-left again. If the first turn already moves the robot to 0.62 meters from the wall, the script still turns left again and may drift to 0.90 meters, away from the hallway center. A feedback policy can instead say: if distance_to_wall > 0.60 meters, steer = +10 degrees and speed_command = 0.5 m/s. In the same second step, it corrects back toward center. The first command can match the script, but the second command is different because the measured state is different. Now make exploration explicit. An epsilon-greedy behavior policy uses the safe steering rule with probability 0.90 and a random legal action with probability 0.10. In state x_near=(0.45,0.8,38), the random branch might choose hard-left at 25 degrees and speed_command = 0.8 m/s. That one exploratory action leads to x_bad=(0.92,0.75,37), close to the far wall. A brittle policy trained only near distance 0.45 may still output hard-left because it never learned what 0.92 means. A recovery policy says: if distance_to_wall > 0.85 meters, steer = +22 degrees and speed_command = 0.2 m/s, which can move the next distance back to 0.70 meters. A greenhouse vent controller makes the script difference plain. It sees x = (temperature 82 F, humidity 70 percent, wind 4 mph) and opens the roof vent to 60 percent because temperature > 80 F. After 5 minutes the greenhouse is 78 F, humidity is 88 percent, and wind rises to 18 mph. A fixed script would keep the vent at 60 percent. A policy reads the new state and can close the roof vent to 20 percent when wind > 15 mph. But if humidity is omitted, that same safe-wind action can let humidity reach 95 percent. A better policy input includes humidity and turns on the side fan at 40 percent while closing the roof vent. The recovery command changed because the measured state changed from x_old to x_new. The next check is whether the fan command keeps humidity below 90 percent at the next measurement, not only whether the wind limit passed. A policy is therefore not only a first action. It is the rule that must survive the states created by its own earlier choices. If the policy ignores battery, humidity, or another needed state variable, it may look reasonable in one frame and fail on the next.",
        "math": "A deterministic policy has the form u = pi(x). A stochastic policy has the form pi(u|x), a distribution over actions conditioned on the current state or observation. In the hallway example, pi(slow-left|x) = 0.8 and pi(hard-left|x) = 0.2. Closed-loop rollout means x_{t+1}=f(x_t,u_t) with u_t drawn from pi(.|x_t) or set to pi(x_t). For a deterministic check, write x_0=(0.45, 0.8, 38). The policy gives u_0=(-18 degrees, 0.4 m/s), and the next measured state is x_1=(0.62, 0.35, 37). The open-loop script uses the stored command u_1=(-18 degrees, 0.4 m/s) again. The policy recomputes u_1=pi(x_1)=(+10 degrees, 0.5 m/s) because x_1 says the robot has moved past the safe middle. That is the difference between a list of actions and a rule. An epsilon-greedy behavior policy can be written pi_b(u|x)=0.90*pi_safe(u|x)+0.10*pi_random(u|x). If pi_random has five legal actions, each random action gets probability 0.10/5 = 0.02 through the exploration branch. The hard-left sample has small probability but real closed-loop consequences: x_bad=f(x_near,hard-left)=(0.92,0.75,37). At x_bad, a recovery policy returns pi_recover(x_bad)=(+22 degrees,0.2 m/s), while the brittle copied policy returns pi_brittle(x_bad)=(-18 degrees,0.4 m/s). The two policies may agree at x_near and disagree at x_bad, so judging only the first-frame action is weak evidence. The policy's quality is judged through the state stream it creates: reward, cost, constraints, and future value are all consequences of repeatedly applying the rule. A policy that omits a needed state variable is a different controller, even if its action looks reasonable in one frame.",
    },
    "value-based-rl": {
        "run": "A warehouse robot reaches a fork while carrying a box. Its state is x = 'two meters from shelf 12, box loaded, battery 22 percent, aisle B blocked.' The two legal actions are turn_left and turn_right. A value-based learner does not first write a full steering policy. It writes a future-return scoreboard for actions. Today the table says Q(x,turn_left) = 3 and Q(x,turn_right) = 7, so the greedy action is turn_right. The robot tries turn_right, gets reward -1 for using another second, and lands in a state where the best stored next-action value is 10. With gamma = 0.9, the new target is y = -1 + 0.9*10 = 8. If the old estimate was 7 and the step size is alpha = 0.5, the updated entry becomes 7 + 0.5*(8 - 7) = 7.5. The action rule has changed because the scoreboard changed. Now make the next state visible. At x_next, the scoreboard says Q(x_next,charge_now)=10 and Q(x_next,inspect_box)=4. The epsilon-greedy behavior policy explores and actually chooses inspect_box, leading to a slow charger line. SARSA would use the action actually taken: y_SARSA = -1 + 0.9*4 = 2.6, so the same old 7 would update to 7 + 0.5*(2.6 - 7) = 4.8. Q-learning instead uses the greedy target action charge_now: y_Q = -1 + 0.9*10 = 8, giving 7.5. Same first transition, different target. Now question the scoreboard. The estimate Q(x_next,charge_now)=10 came from only two smooth-floor samples with returns 11 and 9. After deployment, the robot sees three slick-floor samples for charge_now with returns -4, -2, and 0 because it skids near the charger. The five-sample mean is (11 + 9 - 4 - 2 + 0)/5 = 2.8, not 10. With the repaired next-state value, the Q-learning target for turn_right becomes y_repaired = -1 + 0.9*4 = 2.6 if inspect_box is now the best safe table entry, or -1 + 0.9*2.8 = 1.52 if charge_now remains the compared action. The greedy choice can flip without changing the reward formula. If the estimate for charge_now is high only because the learner has never seen the slick floor after the corner, Q-learning may choose the worse aisle with mathematical confidence.",
        "math": "A state-value function V_pi(x) estimates return starting from state x and then following policy pi. An action-value function Q_pi(x,u) estimates return after taking action u first and then continuing. Value-based reinforcement learning alternates two jobs: policy evaluation, which moves V or Q toward returns or temporal-difference targets, and policy improvement, which chooses actions that look best under the updated values. In a simple Q-learning form, y = r + gamma max_a Q(x_next,a), and Q(x,u) <- Q(x,u) + alpha*(y - Q(x,u)). In the fork example, max_a Q(x_next,a)=10 because charge_now beats inspect_box in the table. The behavior policy is the rule that collected the sample in the real warehouse; here it was epsilon-greedy and picked inspect_box. The target policy is the rule used inside the update; for Q-learning it is greedy, so it prices the next state as if charge_now will be taken. SARSA uses y_SARSA = r + gamma Q(x_next,u_next), where u_next is the action actually taken by the behavior policy, so it gives 2.6 in the explored slow-line sample. Q-learning uses y_Q = r + gamma max_a Q(x_next,a), so it gives 8. The difference is not a naming detail. SARSA learns the value of behaving with exploration still present; Q-learning learns toward the greedy policy while using exploratory data to get coverage. The repaired charge_now table shows the sample side of the method. A Q entry is an estimate, not a fact. Two returns [11,9] give mean 10; adding slick-floor returns [-4,-2,0] gives mean 2.8. If Q(x_next,inspect_box)=4, the max in the TD target changes from 10 to 4, and the target changes from 8 to 2.6. If a step with alpha=0.5 updates the old Q(x,turn_right)=7 toward 2.6, the new value is 7 + 0.5*(2.6 - 7) = 4.8. The boundary is representation honesty and coverage: if the learned Q function generalizes from smooth floors to slick floors without sensing the difference, or if the table has only lucky early samples, argmax will faithfully amplify the wrong number.",
    },
    "policy-optimization": {
        "run": "A small walking robot can choose short_step or long_step. In state x = 'left foot planted, body leaning forward 6 degrees, right knee bent 20 degrees,' its current policy gives pi_theta(long_step|x) = 0.30 and pi_theta(short_step|x) = 0.70. It tries long_step on a rubber mat, travels 0.18 meters, stays upright, and gets return G = 12. It tries short_step in a similar state, travels 0.05 meters, wobbles, and gets return G = 2. The local baseline is baseline b(x)=7, meaning the learner expected about 7 return from this kind of stance before seeing which step was taken. Now the comparison is not just '12 is bigger than 2.' It is A_long = 12 - 7 = +5 and A_short = 2 - 7 = -5. Long_step was better than expected; short_step was worse than expected. Policy optimization does not first fill a Q table and then take argmax. It changes theta so the sampled action with positive advantage becomes more likely in that kind of state, while the sampled action with negative advantage becomes less likely. With a small step, the same state may have pi_theta_new(long_step|x) = 0.38 and pi_theta_new(short_step|x) = 0.62. That is the learned controller changing at the place where actions are chosen. The probability shift is earned by advantage, not by the action name. Now make the damage from a coarse state visible. On rubber, the old expected return from this two-action choice is 0.30*12 + 0.70*2 = 5.0. After the small update it is 0.38*12 + 0.62*2 = 5.8. Later the robot tries long_step on a tile floor, slips, and gets return G = -8. If floor material is missing from the state, the learner treats tile and rubber as the same x. A large update can push pi_theta(long_step|x) down to 0.05 and pi_theta(short_step|x) up to 0.95. Back on rubber, the expected return is then 0.05*12 + 0.95*2 = 2.5. The tile sample may have been honest, but the state was too blunt and the update erased a useful rubber-mat action. If the state includes floor=rubber and floor=tile, the repair can be narrower: keep pi(long_step|rubber) near 0.38 while dropping pi(long_step|tile) near 0.05. If the reward forgot knee impact, the method can make long_step more likely even while wearing out the joint.",
        "math": "The objective is expected return under the policy, written as J(theta)=E_{tau~pi_theta}[G(tau)]. A policy-gradient update estimates g = E[grad_theta log pi_theta(u_t|x_t) * A_t], where A_t is a return, reward-to-go, or advantage telling whether the sampled action was better than the local baseline. Then theta_new = theta + eta*g. In a two-action softmax picture, theta can be read as scores before probabilities are normalized. With eta = 0.04, the long-step sample gives Delta score_long = 0.04*5 = 0.20, while the short-step sample gives Delta score_short = 0.04*(-5) = -0.20. Those score moves do not promise the exact 0.30 to 0.38 probability change; they explain the direction and size of the pressure before the softmax turns scores into probabilities. The transcript's surrogate objective idea is an implementation trick: choose a differentiable loss whose gradient is the policy gradient, then use automatic differentiation to get g. Unlike value-based RL, the policy is explicit; no separate argmax over Q is needed at action time. The boundary is sample honesty and step size. Suppose the robot later tries long_step on a tile floor and gets return G = -8. Against the same baseline, A_tile = -8 - 7 = -15. With eta = 0.20, Delta score = 0.20*(-15) = -3.0, a much larger move. That update may be correct for tile, but rubber mat and tile floor are different states. The probability-ratio check says how far the new controller moved from the controller that collected the data. If old pi(long_step|x)=0.30 and new pi(long_step|x)=0.05, the ratio is 0.05/0.30 = 0.167. If a trust rule only allows ratios from 0.8 to 1.2, that update is too large to treat as a small correction. The expected-return arithmetic shows the same issue: rubber value falls from 0.38*12 + 0.62*2 = 5.8 to 0.05*12 + 0.95*2 = 2.5 when the bad tile sample is applied to the shared state. Splitting the state makes two local updates instead of one: A_rubber=+5 pushes long_step up for rubber, while A_tile=-15 pushes long_step down for tile. Noisy returns can push the probability of a lucky bad action upward, and a large eta can move the policy so far that the collected rollout no longer describes the new controller.",
    },
    "exploration": {
...
CONCEPT_RUN_APPENDIX: dict[str, str] = {
    "feasibility": "Add a second certificate with heat. A rescue drone has 12 battery units and must cross 90 meters of water. Fast flight covers 30 meters per minute and uses 5 battery units per minute; slow flight covers 18 meters per minute and uses 2 units. A fast three-minute trip reaches 90 meters but uses 15 units, so battery residual is 15 - 12 = 3 and the plan is infeasible. A slow five-minute trip uses 10 units and reaches 90 meters, but if the mission deadline is four minutes, it misses time even though battery passes. A mixed plan [fast, slow, slow, slow] covers 30 + 18 + 18 + 18 = 84 meters in four minutes and uses 5 + 2 + 2 + 2 = 11 units, so it still misses distance by 6 meters. The feasible set is empty only after every written family fails every required row: distance, battery, and deadline.",
    "reachability": "A wheelchair robot is 1.4 meters from an elevator door that closes in 3 seconds. It can command 0.4 m/s, but hallway crowding can subtract 0.1 m/s. The safe doorway strip after 3 seconds is [1.0, 1.2] meters from the start. Starting at p = 0.0 and commanding full speed gives best p_3 = 0.0 + 3*0.4 = 1.2 and worst p_3 = 0.0 + 3*(0.4 - 0.1) = 0.9, so the target is not guaranteed. Starting at p = 0.1 gives worst 1.0 but best 1.3, so the robot may hit the door edge. The set answer is not one middle prediction; it is the whole interval of possible next positions, with the bad edge kept visible.",
    "exploration": "A cleaning robot faces a shiny wet tile strip. Its old policy crosses at 0.30 m/s and slips 2 times in 20 crossings, so the stored success estimate is 18/20 = 0.90. Exploration proposes a new action: steer 6 degrees left and slow to 0.22 m/s, but only when the wet-stripe sensor is below 0.40, speed is at most 0.25 m/s, and no person is within 2 meters. In 12 guarded crossings the robot gets 10 clean crossings, 1 slip, and 1 stop, so the raw success rate is 10/12 = 0.83. That looks worse than 0.90 until the states are split. On readings from 0.30 to 0.40, the old action slipped 4 times in 8 tries while the new action slipped 1 time in 7 tries. Exploration was useful because it exposed the state where the old average was hiding risk.",
    "local-quadratic-approximation": "A thermostat starts at 66 F with target 70 F. Around the current heater setting, the controller tests three small moves for the next five minutes: delta heat -1 gives score 19, delta heat 0 gives score 12, and delta heat +1 gives score 9. A fitted bowl through those three points suggests the best move is about +1.6, outside the tested range. The controller does not trust that far step; it takes the guarded move +1. After five minutes the room is 68.5 F and the score is 8.5. The next local tests give 9.4, 8.5, and 8.7. If the model predicted 9.0 and measured 8.5, the local bowl was close enough. If it predicted 6.0 and measured 14.0, the repair is not a slogan about nonlinearity. Shrink the trust region, refit the bowl, and stop treating the old slope as useful.",
    "shooting-methods": "A toy elevator starts at floor 0 with speed 0 and must stop at floor 3 after 3 seconds. A shooting method guesses three one-second acceleration commands, simulates forward, and checks the endpoint. Shot [2, 0, -1] gives speeds 2, 2, 1 and positions 2, 4, 5, so it overshoots badly. Shot [1, 1, -2] gives speeds 1, 2, 0 and positions 1, 3, 3; the endpoint is right, but the speed 2 violates a comfort limit of 1.5. Shot [1, 0.5, -1.5] gives speeds 1, 1.5, 0 and positions 1, 2.5, 2.5; comfort passes, but the car stops half a floor short. Shooting is not guessing until a graph looks smooth. It is forward simulation plus explicit endpoint and path residuals.",
    "stability-under-replanning": "A shower controller replans every 5 seconds with target 38 C. At 7:00 the water is 34 C, so it opens the hot valve from 40 percent to 60 percent. After 5 seconds the water is 37 C. If the controller only sees that 37 is below target, it keeps 60 percent. At 7:00:10 the water reaches 40 C, then a new plan closes to 20 percent, and at 7:00:15 the water falls to 36 C. The controller is chasing its own earlier moves. Use a simple burden B = (water_temp - 38)^2 + 0.2*(valve_percent_change)^2. The move from 40 to 60 at 34 C has B = (34 - 38)^2 + 0.2*(0.20)^2 = 16.008. A useful next state, 37 C with 60 to 50, gives B = 1 + 0.2*(0.10)^2 = 1.002. The bad keep-60 choice reaches 40 C with B = 4 + 0 = 4, so the burden rose instead of shrinking.",
    "value-function": "A library return robot is at the front desk F with one book. It can go to the elevator E at cost 2 or the hallway H at cost 7. The value table says V(E)=18 because the elevator is slow today, and V(H)=6 because the hallway reaches the return slot quickly. The first-step comparison is not nearest-looking door; it is 2 + 18 = 20 against 7 + 6 = 13, so the hallway wins. Now a side door opens beside the elevator and the true remaining value becomes V(E, side_door_open)=4. The elevator route is now 2 + 4 = 6 and should win. If the table stores only V(E)=4 and the side door closes, the real remaining time goes back to 18. The value number must be indexed by the state facts that change the future.",
    "direct-transcription": "A delivery cart path is written at four knot points: x_0, x_1, x_2, x_3. The optimizer chooses all positions and wheel commands together, but each adjacent pair must pass the same physics defect check. Suppose x_0 = 0, x_1 = 1.0, x_2 = 2.4, x_3 = 3.0 with dt = 1 second and speed commands u_0 = 1.0, u_1 = 1.0, u_2 = 0.6. The first defect is x_1 - (x_0 + u_0*dt) = 1.0 - 1.0 = 0. The second is 2.4 - (1.0 + 1.0) = 0.4, so the drawn path teleports 0.4 meters. The third defect is 3.0 - (2.4 + 0.6) = 0. Direct transcription is useful because that bad middle row is an equality violation, not a pretty curve with a hidden lie.",
    "distribution-shift-imitation": "A checkout scanner learns from 5,000 expert scans. In the data, 4,850 cans are flat and centered within 20 cm, 120 are tilted less than 10 degrees, and only 30 are more than 3 cm off center. Held-out accuracy says 97 percent of predicted angles are within 2 degrees, but that held-out set has the same centered mix. In deployment, the learner nudges a soup can 4 cm right. On the mild centered examples, a command of 3 degrees left at 0.4 m/s was fine. Here the expert would use 12 degrees left at 0.1 m/s. After 200 learner scans, 38 frames are off-center, which is 38/200 = 19 percent. The expert data rate was 30/5000 = 0.6 percent. The learner has created the state distribution that the demonstrations barely covered.",
    "costate-adjoint-variable": "A crane hook example shows why the backward price is not just a final error number. At time 1 the hook is 0.4 meters low and moving down at 0.3 m/s. The terminal cost is 80*height_error^2 + 10*velocity_error^2. If one extra meter of height at time 1 reduces final error by 0.8 meters, and one extra meter per second of upward velocity reduces it by 0.5 meters, height and velocity have different backward prices. With terminal gradient [-64, -6], an extra acceleration changes next height by 0.5 and next velocity by 1.0, so the future-price part is -64*0.5 + (-6)*1.0 = -38. If motor heat adds immediate derivative +45, the total derivative becomes +7 and more acceleration is no longer worth it. The costate advice changed when a missing present cost was added.",
    "dynamics": "A conveyor belt shows the same command under different dynamics. A package starts at p = 1.0 m with v = 0.2 m/s. The requested motor command is u = 0.3 m/s for dt = 1.0. On a dry belt with gain 1.0, v_next = 0.3 and p_next = 1.3. On a slippery belt with gain 0.4, v_next = 0.12 and p_next = 1.12. With a one-tick motor delay, the applied command remains old u_prev = 0, so p_next = 1.0 even though the requested command is u = 0.3. Same command string, different dynamics, different next state. A page that hides gain and delay cannot explain why the measured package is late.",
    "trajectory-optimization": "A hospital delivery robot carries a soup tray and samples its path every 0.5 seconds. Candidate A reaches the room on time and keeps tray height legal, but at t = 1.0 its acceleration is 2.4 m/s^2, above the soup limit of 1.2. Candidate B lowers acceleration to 1.0, but passes 0.03 m from a bed when the clearance rule is 0.10 m. Candidate C uses a wider path with clearance 0.14 m and acceleration 1.1, but arrives 0.4 seconds late. Trajectory optimization is the whole history check: clearance, acceleration, timing, and endpoint all have to pass. One good row cannot forgive another failed row.",
    "behavioral-cloning": "A mail-sorting arm copies 2,000 expert moves. Most labels say that a flat envelope at x = 0.0 should move straight forward with wrist angle 0 degrees. Only 25 examples show a curled envelope edge, and the expert uses wrist angle 18 degrees there. A cloned policy scores 98 percent action accuracy because curled edges are rare. On the real line, one small straight push curls the next envelope more, so the arm now sees a state like the rare 25 examples but still predicts 0 degrees. After 100 envelopes, 14 have curled edges caused by the learner itself. The training rate was 25/2000 = 1.25 percent; the learner-created rate is 14 percent. The cloning loss looked good because it tested the expert's state mix, not the states produced by the copied controller.",
    "value-based-rl": "Add one more fork check. A hallway robot has Q(dry, fast)=8 and Q(dry, slow)=5, so fast wins on dry floor. On wet floor it has only one old sample for fast with return 7 and eight samples for slow with mean return 4, so the table still chooses fast. The next real wet trial returns -6 after a skid. The two-sample mean for wet fast becomes (7 + -6)/2 = 0.5. Now slow should win even though no reward rule changed. Value-based control is only as strong as the state split and sample count behind the value being compared.",
    "lqr": "A warehouse cart near the lane center has error e = 0.10 m and speed error 0.20 m/s. The LQR law asks for a small steering correction and behaves like a useful spring. But if the cart is 1.4 m off center, the same linear rule may ask for steering 35 degrees while the actuator limit is 15 degrees. The actual command is clipped, so the next state is not the one the local quadratic model priced. The fix is not to call LQR bad. Use it where the linear model and actuator range are honest, or relinearize and add the real bounds.",
    "bellman-recursion": "A hallway rover has two moves from state S. The smooth shortcut costs 1 now and leads to state A with value 20 because a wheel may jam there. The longer route costs 5 and leads to state B with value 4. Bellman's comparison is 1 + 20 = 21 against 5 + 4 = 9, so the longer route is better. If the value table forgets the damaged-wheel flag and stores V(A)=3 from normal wheels, the shortcut falsely scores 4. The recursion did not fail; the state label feeding the recursion was too poor.",
    "calculus-of-variations": "Add an obstacle to the rail curve. The cart must still move 1 meter in 2 seconds, but between t = 0.8 and t = 1.2 it passes a fragile display, so speed must stay below 0.35 m/s there. The constant-speed curve u(t)=0.5 has the lowest energy without that rule, but it violates the middle interval. A legal curve can move at 0.7 before the display, 0.3 through the display, and 0.5 after it, adjusted so the total area remains 1 meter. The variation test must now use only bumps that preserve the endpoint and do not push the middle interval over its speed limit.",
    "hamiltonian-optimal-control": "A small heat term changes the local Hamiltonian choice. With H(u)=0.5*u^2 - 20*u, the unconstrained minimum is u = 20 and a cap at 12 makes u = 12 attractive. Now suppose thrust above 9 heats a battery, with added cost 4*(u - 9)^2 when u is above 9. At u = 12 that heat bill is 4*9 = 36. At u = 9 there is no heat bill. The old local account preferred 12 because it counted speed but not heat. Once heat is in the Hamiltonian, the derivative at 12 is no longer the same local advice.",
    "model-based-rl": "A second planning test catches model exploitation. The learned cart model has never seen cardboard dust near the shelf, but the optimizer discovers that braking 70 percent late gives a short predicted stop. The ensemble margins are 0.22, 0.21, 0.05, -0.03, and 0.24 meters. The average is below the 0.20 m rule once the bad models are counted, and the negative margin says one plausible model hits the shelf. A careful model-based learner rejects that sequence, tries a milder first action on hardware, logs the real transition, and replans from the measured state.",
}


def concept_run(concept: dict[str, Any]) -> dict[str, str]:
    base = CONCEPT_RUNS[concept["id"]]
    appendix = CONCEPT_RUN_APPENDIX.get(concept["id"])
    if not appendix:
        return base
