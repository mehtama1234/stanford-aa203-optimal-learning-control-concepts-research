...
def main() -> int:
    ...
    for concept in concepts:
        ...
        if concept.get("id") == "model-predictive-control":
            for marker in ["3.0 meters from a loading mark", "10:00:00", "-0.8 m/s^2", "v_next = v + dt*u", "p_next = p + dt*v_next", "velocity 0.60 m/s and position 3.30 meters", "3.53 meters and 3.70 meters", "old -0.6 and -0.2", "position 3.40 meters and velocity 0.50 m/s", "not the predicted 3.30 meters and 0.60 m/s", "[-0.4, -0.3, -0.1]", "v_next = 0.50 + 0.5*(-0.4) = 0.30 m/s", "p_next = 3.40 + 0.5*0.30 = 3.55 meters", "u_0...u_{N-1}", "x_0 = x_measured(k)", "pi_MPC(x_measured(k)) = u_0^*", "x_0 = x_measured(k+1)", "old tail [-0.6, -0.2]", "predicted state (3.30, 0.60)", "measured state (3.40, 0.50)", "chooses new tail [-0.4, -0.3, -0.1]", "applied control is therefore -0.4, not the old -0.6", "solve takes 0.8 seconds", "control period is 0.5 seconds", "no feasible continuation", "Recursive feasibility and stability are extra promises"]:
                if marker not in text:
                    errors.append(f"MPC concept page missing receding-horizon marker: {marker}")
            if len(words) < 980:
                errors.append(f"MPC concept page below core richness floor: {len(words)} < 980")
