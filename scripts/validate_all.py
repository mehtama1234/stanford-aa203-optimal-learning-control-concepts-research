...
def main() -> int:
    ...
    for concept in concepts:
        ...
        if concept.get("id") == "action-control-input":
            for marker in ["h = 10 meters", "v = 0.2 m/s", "48 percent above hover", "dt = 0.1 seconds", "a(u)=4.8 m/s^2", "v_next = 0.2 + 0.1*4.8 = 0.68 m/s", "h_next = 10 + 0.1*0.68 = 10.068 meters", "6.8 centimeters", "u = 0.9, meaning 90 percent above hover", "u_sent = 0.5", "0.2 + 0.1*5.0 = 0.70 m/s", "next height is 10.070 meters", "u_prev = 0.10", "change thrust by only 0.20 per tick", "u_rate = 0.30", "a(u_rate)=3.0 m/s^2", "v_next = 0.2 + 0.1*3.0 = 0.50 m/s", "h_next = 10 + 0.1*0.50 = 10.050 meters", "limits, rate changes, motors, acceleration, velocity, and only then position", "a(u)=10*u", "-0.3 &lt;= u &lt;= 0.5", "v_next = v + dt * a(u)", "h_next = h + dt * v_next", "u_clipped = min(0.9,0.5) = 0.5", "a=5.0 m/s^2", "v_next = 0.2 + 0.1*5.0 = 0.70", "h_next = 10 + 0.1*0.70 = 10.070", "|u - u_prev| &lt;= 0.20", "u_rate = 0.30 for this tick", "a(u_rate)=3.0", "v_next = 0.2 + 0.1*3.0 = 0.50", "h_next = 10 + 0.1*0.50 = 10.050", "x_next = f(x, u_rate)", "command that actually reaches the plant", "(h_next - 12)^2", "actuator truth", "delay, saturation, dead zones, and rate limits"]:
                if marker not in text:
                    errors.append(f"action concept page missing command-vs-outcome marker: {marker}")
            if len(words) < 1120:
                errors.append(f"action concept page below core richness floor: {len(words)} < 1120")
        if concept.get("id") == "dynamics":
            for marker in ["speed 20 m/s", "theta = 0 degrees", "u = 5 degrees", "dt = 0.2 seconds", "yaw_rate = 0.30 rad/s", "theta_next = 0 + 0.2*0.30 = 0.06 rad", "yaw_rate = 0.05 rad/s", "v_y = -1.0 m/s", "theta_next = 0 + 0.2*0.05 = 0.01 rad", "y_next = y + 0.2*(-1.0)", "three ticks", "theta = 0.18 rad after 0.6 seconds", "theta = 0.03 rad", "y = -0.6 meters", "u_now = 5 degrees", "u_prev = 0 degrees", "first yaw rate is 0.00 rad/s", "theta_next stays 0.00 rad", "following tick, not this one", "x_next = f(x,u)", "dx/dt = f(x,u)", "x=[y,theta,v_y,speed]", "f_dry", "f_ice", "theta_3 = 3*0.2*0.30 = 0.18 rad", "theta_3 = 3*0.2*0.05 = 0.03 rad", "y_3 = 3*0.2*(-1.0) = -0.6 meters", "u_applied = u_prev", "theta_next = theta + dt*yaw_rate(x,u_prev,grip)", "yaw_rate = 0.00 rad/s", "theta_next = 0 + 0.2*0.00 = 0.00 rad", "u_now = 5 degrees has just been requested", "carry u_now forward as the future u_prev", "one-tick delay", "model fidelity"]:
                if marker not in text:
                    errors.append(f"dynamics concept page missing state-update marker: {marker}")
            if len(words) < 1150:
                errors.append(f"dynamics concept page below core richness floor: {len(words)} < 1150")
        if concept.get("id") == "objective-cost-function":
            for marker in ["Path A reaches the spot in 8 seconds", "Path B takes 12 seconds", "8 is smaller than 12", "steering effort 10", "wall risk 30", "steering effort 3", "Path A: 8 + 0.5*10 + 1.0*30 = 43", "Path B: 12 + 0.5*3 + 1.0*0 = 13.5", "wall weight 0", "Path A scores 13 and beats 13.5", "thermostat from the course overview", "room target is 70 F", "temperatures 69, 70, 71", "uses 3 heater units", "Plan B keeps 67, 68, 70", "uses 1 heater unit", "temperature_error^2 + 0.2*heat", "total 2.6", "total 13.2", "energy weight rises to 5.0", "Plan A adds 15 and scores 17", "Plan B adds 5 and scores 18", "terminal cost pays 20*(final_error)^2", "B wins despite worse early tracking", "written measurement of what counts as better", "J = sum_k stage_cost(x_k,u_k) + terminal_cost(x_N)", "w_wall*wall_risk", "stage_0 + stage_1 + stage_2", "0.5*steering_effort", "A has base score 8 + 0.5*10 = 13", "B has 12 + 0.5*3 = 13.5", "30*w_wall", "13 + 30*w_wall &lt; 13.5", "w_wall &lt; 0.5/30 = 0.0167", "Any wall weight above 0.0167 flips the written choice to B", "stage_cost_k=(temp_k - 70)^2 + alpha*heat_k", "terminal_cost=beta*(temp_3 - 70)^2", "alpha=0.2 and beta=0", "J_A=(1+0+1)+0.2*3=2.6", "J_B=(9+4+0)+0.2*1=13.2", "alpha=5 and beta=0", "J_A=2+15=17", "J_B=13+5=18", "alpha=5 and beta=20", "J_A=17+20*(1^2)=37", "J_B=18+20*(0^2)=18", "written exchange rates changed", "follows the score", "proxy honesty"]:
                if marker not in text:
                    errors.append(f"objective concept page missing tradeoff marker: {marker}")
            if len(words) < 1160:
                errors.append(f"objective concept page below core richness floor: {len(words)} < 1160")
        if concept.get("id") == "horizon":
            for marker in ["60 meters from the pad", "18 percent battery", "full-speed flight uses 2 percent battery", "slow flight uses 1 percent battery", "A three-second horizon sees only the next 24 meters", "crosswind band from 25 meters to 40 meters", "at least 14 percent battery", "five-second horizon", "distance_5 = 5*8 = 40 meters", "battery_5 = 18 - 5*2 = 8 percent", "violates the 14 percent reserve rule", "ten-second horizon", "Full speed would travel 80 meters", "overshooting the 60 meter pad by 20 meters", "slow speed would be only 40 meters", "neither simple plan is acceptable", "shaped speed plan", "charging stop, or a different route", "distance_3 = 3*8 = 24 meters", "battery_3 = 18 - 3*2 = 12 percent", "distance_3 = 3*4 = 12 meters", "battery_3 = 18 - 3*1 = 15 percent", "At 5 seconds", "distance_5 = 40 meters", "battery_5 = 8 percent", "battery_at_wind = 8 &lt; 14", "At 10 seconds", "distance_10 = 10*8 = 80 meters", "overshoot_10 = 80 - 60 = 20 meters", "distance_10 = 10*4 = 40 meters", "battery_10 = 18 - 10*1 = 8 percent", "battery_at_wind &gt;= 14 percent", "dt = 0.5 seconds", "N = 60", "Too short is blind; too long can be expensive or misleading", "first consequence time", "first action still leaves a legal future"]:
                if marker not in text:
                    errors.append(f"horizon concept page missing delayed-consequence marker: {marker}")
            if len(words) < 1150:
                errors.append(f"horizon concept page below core richness floor: {len(words)} < 1150")
        if concept.get("id") == "constraints":
            for marker in ["4 centimeters from the shelf", "12 newton-meters", "t = 0.4 seconds", "3 centimeters from the shelf", "t = 0.6 seconds", "15 newton-meters", "5 centimeters", "11 newton-meters", "3 &lt; 4 and 15 &gt; 12", "2 points per missing centimeter", "Candidate C uses only 4 energy points", "2 centimeters from the shelf", "4 + 2*(4 - 2) = 8", "8 &lt; 11", "Candidate C must still be rejected", "wrist would like torque 14", "minimizes (tau - 14)^2", "tau = 14 illegal", "best legal torque is tau = 12", "upper torque constraint is active", "tau = 10", "2 newton-meters of slack", "From tau = 12", "move upward", "leaves the feasible interval", "both small upward and downward changes are still legal", "g(x,u) &lt;= 0", "g_clear,k = 0.04 - distance_to_shelf(x_k) &lt;= 0", "distance_to_shelf = 0.03", "g_clear = 0.04 - 0.03 = 0.01 &gt; 0", "g_tau,k = abs(tau_k) - 12 &lt;= 0", "g_tau = 15 - 12 = 3 &gt; 0", "g_clear = -0.01", "g_tau = -1", "Candidate C has distance 0.02", "g_clear = 0.04 - 0.02 = 0.02 &gt; 0", "rho*max(0,g_clear)", "100*0.02 = 2", "not the same statement as g_clear &lt;= 0", "J(tau)=(tau - 14)^2", "-12 &lt;= tau &lt;= 12", "unconstrained minimizer is tau = 14", "violates tau &lt;= 12", "g_upper(tau)=tau - 12 = 0", "upper bound is active", "g_upper(tau)=10 - 12 = -2", "bound is inactive", "defect_k = x_{k+1} - f(x_k,u_k) = 0", "shelf flex, cable snag, heat, or human clearance"]:
                if marker not in text:
                    errors.append(f"constraints concept page missing hard-limit marker: {marker}")
            if len(words) < 1160:
                errors.append(f"constraints concept page below core richness floor: {len(words)} < 1160")
        if concept.get("id") == "feasibility":
            for marker in ["18 meters behind a stopped truck", "22 m/s", "wet-road braking limit of 6 m/s^2", "v^2/(2a) = 22^2/(2*6) = 40.3 meters", "0.5 meters of side clearance", "safety rule requires 1.5 meters", "concrete barrier is 0.8 meters away", "45 meters behind the truck", "40.3 &lt;= 45 is true", "4.7 meters of stopping margin", "feasible set is not empty", "cheap collision path is not a plan with a high cost", "legal shoulder opening 12 meters ahead", "did not include the shoulder lane as an action family", "two-step escape", "brake hard for 0.5 seconds", "dropping speed from 22 to 19 m/s", "using about 10 meters", "2.0 meters of side clearance", "old empty set was true for the old model", "richer action set", "x_{k+1}=f(x_k,u_k)", "stop_distance &lt;= 18", "40.3 &lt;= 18 is false", "clearance_left &gt;= 1.5", "0.5 &gt;= 1.5 is false", "clearance_right &gt;= 1.5", "0.8 &gt;= 1.5 is false", "F_old(x_0) = empty set", "x_safe with truck_distance = 45 meters", "stop_distance &lt;= 45", "F(x_safe) contains at least the full-brake plan", "u_shoulder", "v_1 = 22 - 6*0.5 = 19 m/s", "d_used = 0.5*(22 + 19)*0.5 = 10.25 meters", "18 - 10.25 = 7.75 meters", "shoulder clearance is 2.0 &gt;= 1.5", "u_shoulder belongs to F_new(x_0)", "legal candidate to compare", "J = 10,000", "illegal plan with J = 1", "model honesty and horizon length", "forgot a shoulder lane"]:
                if marker not in text:
                    errors.append(f"feasibility concept page missing empty-set marker: {marker}")
            if len(words) < 1160:
                errors.append(f"feasibility concept page below core richness floor: {len(words)} < 1160")
        if concept.get("id") == "value-function":
            for marker in ["state S", "state R", "V(R)=18", "state H", "V(H)=7", "1 + 18 = 19", "4 + 7 = 11", "same V(R)=18", "V_repaired(R)=9", "1 + 9 = 10", "beats smooth 11", "different road also reaches R", "f(S,rocky)=R", "f(S,smooth)=H", "Q(S,rocky)=1+V(R)=1+18=19", "Q(S,smooth)=4+V(H)=4+7=11", "V(S)=11", "Q(S,rocky)=1+9=10", "V(S)=10", "V(R)=6", "1 + 6 = 7", "V(x)=min_u [cost(x,u) + V(f(x,u))]", "reward version", "battery heat, wheel damage, traffic, or a locked gate", "repair station, cost, or allowed policy changes"]:
                if marker not in text:
                    errors.append(f"value-function concept page missing future-price marker: {marker}")
            if len(words) < 980:
                errors.append(f"value-function concept page below core richness floor: {len(words)} < 980")
        if concept.get("id") == "bellman-recursion":
            for marker in ["state x = J", "V(C)=9", "V(L)=3", "2 + V(C) = 2 + 9 = 11", "5 + V(L) = 5 + 3 = 8", "V(J)=8", "pi(J)=B", "V(L)=14", "B to 5 + 14 = 19", "junction switches to A", "time t=1", "shelf state S", "V_2(goal)=0", "V_2(wrong_shelf)=40", "4 + 0 = 4", "1 + 40 = 41", "V_1(S)=4", "pi_1(S)=slow_scan", "stored numbers are consistent", "Bellman recursion is the one-step accounting identity", "V(x) = min_u [c(x,u) + V(f(x,u))]", "pi(x) = argmin_u [c(x,u) + V(f(x,u))]", "Q(J,A)=2+V(C)=11", "Q(J,B)=5+V(L)=8", "V(J)=min(11,8)=8", "Q(J,B)=5+14=19", "V(J)=min(11,19)=11", "pi(J)=A", "consistency equation", "V(goal)=0", "V_k(x)=min_u [c_k(x,u)+V_{k+1}(f_k(x,u))]", "Q_1(S,slow_scan)=4+V_2(goal)=4", "Q_1(S,rush)=1+V_2(wrong_shelf)=41", "V_1(S)=min(4,41)=4", "time may need to be part of the state", "forgot the deadline", "left side V(x) and the right side min expression agree", "sum_{x_next} P(x_next|x,u)V(x_next)", "stale map"]:
                if marker not in text:
                    errors.append(f"bellman concept page missing recursion marker: {marker}")
            if len(words) < 1120:
                errors.append(f"bellman concept page below core richness floor: {len(words)} < 1120")
        if concept.get("id") == "stochastic-dynamic-programming":
            for marker in ["disturbance W", "probability 0.6", "W = straight", "probability 0.3", "W = slip-left", "probability 0.1", "W = slip-right", "probabilities must add to 1.0", "0.6*5 + 0.3*12 + 0.1*20 = 8.6", "1 + 8.6 = 9.6", "4 + 0.9*4 + 0.1*9 = 8.5", "slip-right is serious but not enormous, say value 60", "1 + 0.6*5 + 0.3*12 + 0.1*60 = 13.6", "broken axle with repair cost 200", "1 + 0.6*5 + 0.3*12 + 0.1*200 = 27.6", "P(straight)=0.68", "P(slip-left)=0.30", "P(slip-right)=0.02", "1 + 0.68*5 + 0.30*12 + 0.02*20 = 8.4", "50 gravel crossings", "24/50 = 0.48", "18/50 = 0.36", "8/50 = 0.16", "1 + 0.48*5 + 0.36*12 + 0.16*20 = 10.92", "detour wins again", "V(x) = min_u [cost(x,u) + E_W V(f(x,u,W))]", "P(x_next|x,u) V(x_next)", "P(straight|x,gravel)=0.6", "P(slip-left|x,gravel)=0.3", "P(slip-right|x,gravel)=0.1", "P(straight|x,gravel)+P(slip-left|x,gravel)+P(slip-right|x,gravel)=1.0", "0.1*20 = 2", "0.1*60 = 6", "0.1*200 = 20", "Q_dry(x,gravel)=1+0.68*5+0.30*12+0.02*20=8.4", "P_hat(straight)=24/50=0.48", "P_hat(slip-left)=18/50=0.36", "P_hat(slip-right)=8/50=0.16", "Q_wet(x,gravel)=1+0.48*5+0.36*12+0.16*20=10.92", "transition model changed", "wet gravel and dry gravel share one state label", "rare catastrophic outcomes", "With slip-right value 60, expectation ranks gravel at 13.6 minutes", "P(broken_axle) &lt;= 0", "reject gravel regardless of its average", "hard safety constraint or a risk measure"]:
                if marker not in text:
                    errors.append(f"stochastic DP concept page missing expectation marker: {marker}")
            if len(words) < 1180:
                errors.append(f"stochastic DP concept page below core richness floor: {len(words)} < 1180")
        if concept.get("id") == "dynamic-programming":
            for marker in ["V(G)=0", "V(A)=1", "V(R)=6", "1 + V(A) = 2", "3 + V(R) = 9", "V(M)=2", "2 + V(M) = 4", "1 + V(R) = 7", "V(S)=4", "S-east-M-right-A-goal", "S-south-R-slow-goal", "does not reopen M", "feedback table", "V_new(R)=2", "3 + V_new(R) = 5", "1 + V_new(R) = 3", "V_new(S)=3", "pi_new(S)=south", "changed future price travels backward", "V(s) = min_a [c(s,a) + V(next(s,a))]", "pi(M)=right", "pi(S)=east", "closed-loop policies", "min(1+V(A), 3+V_new(R)) = min(2,5) = 2", "V_new(M)=2", "pi_new(M)=right", "min(2+V_new(M), 1+V_new(R)) = min(4,3) = 3", "local value change be propagated", "100*5*4 = 2,000 state entries", "100*5*4*20*10 = 400,000 entries", "every entry may need an action comparison", "omits mud depth or battery health", "honest and small enough to cover"]:
                if marker not in text:
                    errors.append(f"dynamic programming concept page missing backward-update marker: {marker}")
            if len(words) < 1080:
                errors.append(f"dynamic programming concept page below core richness floor: {len(words)} < 1080")
        if concept.get("id") == "trajectory-optimization":
            for marker in ["1.2 seconds", "t = 0.0, 0.3, 0.6, 0.9, and 1.2 seconds", "foot height samples are 0.04, 0.18, 0.22, 0.12, and 0.00 meters", "box top is 0.16 meters", "torso lean is 17 degrees", "support limit is 12 degrees", "46 newton-meters", "40 newton-meters", "samples 0.04, 0.20, 0.26, 0.16, 0.00", "torso lean peaks at 11 degrees", "knee torque peaks at 38 newton-meters", "whole motion is legal", "whole state-action history", "feedback tracker", "x_0...x_N and u_0...u_{N-1}", "J = sum_k [10*foot_error_k^2 + 0.01*torque_k^2]", "x_{k+1}=f(x_k,u_k)", "defect_k = x_{k+1} - f(x_k,u_k)", "defect_k = 0", "torso_lean_2 = 17 degrees", "torso_lean &lt;= 12 degrees", "torque_3 = 46", "torque &lt;= 40", "torso_lean_2 = 11 degrees", "torque_3 = 38", "0.26 meters is higher than needed", "trade a slightly higher foot path for lower torque or better balance", "grid skips the instant", "contact model lies about foot slip"]:
                if marker not in text:
                    errors.append(f"trajectory optimization concept page missing path-history marker: {marker}")
            if len(words) < 980:
                errors.append(f"trajectory optimization concept page below core richness floor: {len(words)} < 980")
        if concept.get("id") == "direct-transcription":
            for marker in ["0.0 rad to 1.2 rad", "5 newton-meter torque limit", "t = 0.0, 0.2, 0.4, and 0.6 seconds", "q_0=0.0, q_1=0.4, q_2=0.9, q_3=1.2", "tau_1=2 newton-meters", "q_2_pred = 0.6 rad", "q_2 - q_2_pred = 0.9 - 0.6 = 0.3 rad", "tau_1=8", "8 &gt; 5", "legal repair", "q_2=0.6 while keeping tau_1=2", "q_2 - q_2_pred = 0.6 - 0.6 = 0", "tau_1=5 predicts q_2_pred = 0.75", "Moving q_2 from 0.9 to 0.75", "zero defect and still obeys the torque bound", "Keeping q_2=0.9 would need tau_1=8", "fixes dynamics on paper but breaks the actuator limit", "endpoint q_3=1.2 did not excuse a false middle point", "checks the route, not only the finish", "middle states and controls together", "x_k=[q_k,v_k]", "u_k=tau_k", "defect_k = x_{k+1} - step(x_k,u_k)", "defect_1 = q_2 - q_2_pred = 0.3 rad", "illegal torque repair has tau_1=8", "-5 &lt;= tau_k &lt;= 5", "state repair has q_2=0.6", "defect_1 = 0.6 - 0.6 = 0", "tau_1 remains inside the torque bound", "mixed repair", "tau_1=5 gives q_2_pred=0.75", "choosing q_2=0.75 gives defect_1 = 0.75 - 0.75 = 0", "active torque boundary", "q_2=0.9 with the same legal tau_1=5", "defect_1 = 0.9 - 0.75 = 0.15", "grid state still lies", "If only q_0=0 and q_3=1.2 were checked", "false middle value q_2=0.9 could survive", "written proof that the middle step came from the model", "defect_k = 0", "q_0=0 and q_3=1.2", "states as variables", "stop those exposed states from lying", "between t = 0.2 and t = 0.4"]:
                if marker not in text:
                    errors.append(f"direct transcription concept page missing defect marker: {marker}")
            if len(words) < 1160:
                errors.append(f"direct transcription concept page below core richness floor: {len(words)} < 1160")
        if concept.get("id") == "shooting-methods":
            for marker in ["x = 0 meters", "u_0 = 4 m/s^2", "u_1 = -1 m/s^2", "x_2 = 7 meters", "3 meters short and still moving", "u_0 = 6 m/s^2", "u_1 = -2 m/s^2", "v_1 = 6", "x_1 = 6", "v_2 = 4", "x_2 = 10", "fails the stop condition", "corrected shot is u_0 = 10 m/s^2 and u_1 = -10 m/s^2", "v_1 = 10", "x_1 = 10", "v_2 = 0", "both endpoint errors are zero", "not automatically best", "|u_k| &lt;= 8", "both position and speed", "only u_0 and u_1 are decision variables", "v_{k+1}=v_k + dt*u_k", "x_{k+1}=x_k + dt*v_{k+1}", "residual r = [x_2 - 10, v_2 - 0]", "r = [7 - 10, 3 - 0] = [-3, 3]", "r = [10 - 10, 4 - 0] = [0, 4]", "u=[10,-10]", "r=[10 - 10, 0 - 0]=[0,0]", "J_u = u_0^2 + u_1^2", "10^2 + (-10)^2 = 200", "6^2 + (-2)^2 = 40", "misses the stop condition", "successful endpoint shot is illegal", "r is zero", "respecting bounds", "gap_1 = x_join - step(x_0,u_0)", "u_0=4 then step(x_0,u_0) gives x_1=4 and v_1=4", "x_join=(5,4)", "join gap is [5 - 4, 4 - 4] = [1,0]", "two pieces do not connect", "x_join=(4,4)", "join gap is [0,0]", "state the first piece actually reaches"]:
                if marker not in text:
                    errors.append(f"shooting methods concept page missing forward-shot marker: {marker}")
            if len(words) < 1160:
                errors.append(f"shooting methods concept page below core richness floor: {len(words)} < 1160")
        if concept.get("id") == "collocation":
            for marker in ["x = 0.02 meters", "x = 0.18 meters", "t = 0.2 seconds", "x = 0.10 meters", "x = 0.08 to 0.12 meters", "path_derivative_mid = 0.9 m/s", "f(x_mid,u_mid)=0.4 m/s", "curve is trying to pass through", "x_mid = 0.14 meters", "outside the fixture", "f(x_mid,u_mid)=0.6 m/s", "curve slope is 0.6 m/s", "passes both checks", "one honest midpoint is not a free pass", "t = 0.3 seconds", "x_3 = 0.16 meters", "clearance_3 = 0.16 - 0.12 = 0.04 meters", "defect_3 = 0.5 - 0.5 = 0", "x_3 = 0.11 meters", "fixture collision has merely moved later", "selected interior points to be physically honest", "defect_mid = path_derivative_mid - f(x_mid,u_mid)", "defect_mid = 0.9 - 0.4 = 0.5 m/s", "driving defect_mid to 0", "clearance_mid &gt;= 0", "x_mid = 0.10 meters", "clearance_mid is negative", "clearance_mid = 0.14 - 0.12 = 0.02 meters", "defect_mid = 0.6 - 0.6 = 0", "clearance_3 = x_3 - 0.12", "defect_3 = path_derivative_3 - f(x_3,u_3)", "clearance_3 = 0.04", "clearance_3 = -0.01", "candidate fails even if the midpoint looked repaired", "t = 0.1, 0.2, and 0.3 seconds", "sampling and representation", "unmodeled flex, backlash, or contact event"]:
                if marker not in text:
                    errors.append(f"collocation concept page missing midpoint marker: {marker}")
            if len(words) < 1120:
                errors.append(f"collocation concept page below core richness floor: {len(words)} < 1120")
        ...
        if concept.get("id") == "local-quadratic-approximation":
            for marker in ["6 centimeters too far left", "delta u = -0.02 meters", "delta u = 0.02 meters", "jump 12 centimeters right", "2.8 centimeter suggested step", "slightly outside the tested band", "clip the first move to delta u = 0.02 meters", "real score is measured as 3.15", "predicted 3.1", "extra nudges -0.01, 0, and +0.01 meters", "3.40, 3.15, and 3.05", "another 1.2 centimeter move", "fit nearby, step guardedly, measure again", "q(delta u) = c + g*delta u + 0.5*H*delta u^2", "delta u* = -g/H = 0.028 meters", "q(0.028)=4.0 - 70*0.028 + 0.5*2500*0.028^2 = 3.02", "-0.02 &lt;= delta u &lt;= 0.02", "trust region with radius 0.02", "delta u_clipped = 0.02", "q(0.02)=4.0 - 1.4 + 0.5 = 3.1", "remeasure and refit", "c2 = 3.15", "g2 = -10", "H2 = 833", "g2 + H2*delta u = 0", "delta u2* = 10/833 = 0.012 meters", "center of the approximation moved", "linearize the dynamics", "quadratize the cost or Q function", "trust region"]:
                if marker not in text:
                    errors.append(f"local quadratic approximation concept page missing fitted-bowl marker: {marker}")
            if len(words) < 1150:
                errors.append(f"local quadratic approximation concept page below core richness floor: {len(words)} < 1150")
        if concept.get("id") == "reachability":
            for marker in ["5 meters apart", "0.6 meters per second", "0.4 meters per second", "current lateral gap is 0.5 meters", "0.5 + 0.4 - 0.6 = 0.3", "current gap is 0.4 meters", "best escape leaves 0.2 meters", "gap to 1.0 meters", "step backward one more second", "0.7 + 0.4 - 0.6 = 0.5", "0.5 is already in the one-step danger set", "0.7 is unsafe over a two-second horizon", "0.8 meter gap", "0.8 remains outside this two-step danger map", "T_bad = {gap &lt;= 0.3 meters}", "exists a disturbance w", "for every legal control u", "The order matters", "next_gap &lt;= 0.5 + 0.4 - 0.6 = 0.3", "backward avoidance set A_1", "next_gap &lt;= 0.4 + 0.4 - 0.6 = 0.2", "strictly inside T_bad", "A_2 contains states whose worst-case next state lands in A_1", "A_1 begins at gap &lt;= 0.5", "gap = 0.7 enters A_2", "gap = 0.8 does not enter A_2", "backward part of backward reachability", "repeatedly ask which earlier states can be forced into the set", "for all disturbances, there must exist a control"]:
                if marker not in text:
                    errors.append(f"reachability concept page missing set-propagation marker: {marker}")
            if len(words) < 1160:
                errors.append(f"reachability concept page below core richness floor: {len(words)} < 1160")
        if concept.get("id") == "model-predictive-control":
            for marker in ["3.0 meters from a loading mark", "10:00:00", "-0.8 m/s^2", "v_next = v + dt*u", "p_next = p + dt*v_next", "velocity 0.60 m/s and position 3.30 meters", "3.53 meters and 3.70 meters", "old -0.6 and -0.2", "position 3.40 meters and velocity 0.50 m/s", "not the predicted 3.30 meters and 0.60 m/s", "old next brake -0.6", "v_next = 0.50 + 0.5*(-0.6) = 0.20 m/s", "p_next = 3.40 + 0.5*0.20 = 3.50 meters", "leaving the cart too far short", "[-0.4, -0.3, -0.1]", "v_next = 0.50 + 0.5*(-0.4) = 0.30 m/s", "p_next = 3.40 + 0.5*0.30 = 3.55 meters", "3.625 meters and 3.675 meters", "closer to the mark while still below the stop line", "u_0...u_{N-1}", "x_0 = x_measured(k)", "pi_MPC(x_measured(k)) = u_0^*", "x_0 = x_measured(k+1)", "old tail [-0.6, -0.2]", "predicted state (3.30, 0.60)", "measured state (3.40, 0.50)", "chooses new tail [-0.4, -0.3, -0.1]", "applied control is therefore -0.4, not the old -0.6", "using old -0.6 gives predicted state (3.50, 0.20)", "Using new -0.4 gives predicted state (3.55, 0.30)", "new three-step rollout ends at (3.675, 0.10)", "old one-step position error is 4.0 - 3.50 = 0.50 meters", "new one-step position error is 4.0 - 3.55 = 0.45 meters", "old tail is not sacred", "solve takes 0.8 seconds", "control period is 0.5 seconds", "no feasible continuation", "Recursive feasibility and stability are extra promises"]:
                if marker not in text:
                    errors.append(f"MPC concept page missing receding-horizon marker: {marker}")
            if len(words) < 1120:
                errors.append(f"MPC concept page below core richness floor: {len(words)} < 1120")
        if concept.get("id") == "recursive-feasibility":
            for marker in ["2.0 meters left before the stop line", "speed is below 0.2 m/s", "1.4 m/s with only 1.1 meters left", "maximum braking 0.8 m/s^2", "v^2/(2a)=1.4^2/(2*0.8)=1.225 meters", "more than the 1.1 meters left", "handed 10:00.5 an impossible problem", "speed 1.0 m/s with 1.2 meters left", "1.0^2/(2*0.8)=0.625 meters", "0.575 meters of margin", "measured successor is worse than planned", "speed 1.1 m/s with 1.15 meters left", "1.1^2/(2*0.8)=0.756 meters", "leaving 0.394 meters", "below the required 0.4 meter terminal margin", "measured handoff failed by 0.006 meters", "feasible continuation", "[u_0^*, u_1^*, u_2^*]", "x_3 lies in a terminal set X_F", "x_{k+1}=f(x_k,u_0^*) = x_1", "[u_1^*, u_2^*, v_backup]", "X_F is controlled invariant", "f(x_3,v_backup) in X_F", "speed &lt;= 0.2 m/s", "stopping_distance_remaining &gt;= 0.4 meters", "outside the safe handoff set", "even maximum braking needs 1.225 meters", "stopping_distance = 0.625 meters", "stopping_distance_remaining - stopping_distance = 1.2 - 0.625 = 0.575 meters", "still has a continuation", "stopping_distance = 1.1^2/(2*0.8)=0.756 meters", "stopping_distance_remaining - stopping_distance = 1.15 - 0.756 = 0.394 meters", "0.394 &lt; 0.4", "measured state is outside X_F", "planned x_1 was inside", "not invariant"]:
                if marker not in text:
                    errors.append(f"recursive feasibility concept page missing shifted-tail marker: {marker}")
            if len(words) < 1150:
                errors.append(f"recursive feasibility concept page below core richness floor: {len(words)} < 1150")
        if concept.get("id") == "stability-under-replanning":
            for marker in ["1.5 meters left", "E = distance_error^2 + 0.5*sideways_speed^2", "E = 1.5^2 + 0.5*0.8^2 = 2.57", "E = 0.9^2 + 0.5*0.5^2 = 0.935", "0.5 meters left with sideways speed 0.3 m/s right", "E = 0.5^2 + 0.5*0.3^2 = 0.295", "burden keeps falling", "1.0 meters right", "sideways speed 0.6 m/s left", "E = 1.0^2 + 0.5*0.6^2 = 1.18", "burden rose from 0.935 to 1.18", "one-step horizon", "tiny brake", "leaves E = 0.90", "leave E = 0.88", "loading dock requires E &lt;= 0.20 within three seconds", "not spending error fast enough", "terminal condition E_terminal &lt;= 0.20", "terminal cost 6*E_terminal", "microscopic progress", "terminal cost, terminal set, or a decrease condition", "x_{k+1}=f_closed(x_k)", "V(f_closed(x)) - V(x) &lt;= -stage_cost(x,pi_MPC(x))", "delta V = 0.935 - 2.57 = -1.635", "delta V = 0.295 - 0.935 = -0.640", "delta V = 1.18 - 0.935 = +0.245", "violates a nonincrease test", "stage cost for the second state were 0.10", "delta V &lt;= -0.10", "accept -0.640 and reject +0.245", "nonincrease alone may be too weak", "E_0=0.935", "E_1=0.90", "E_2=0.88", "E_{k+1} &lt;= E_k", "miss the required E_3 &lt;= 0.20 target", "terminal constraint writes E_N &lt;= 0.20", "sum stage_cost + 6*E_N", "E_N=0.88 adds 5.28 points", "E_N=0.18 adds 1.08 points", "Feasibility alone would only say the next optimization exists", "terminal set gives the final predicted state", "cycling, drifting, delaying, or amplifying velocity"]:
                if marker not in text:
