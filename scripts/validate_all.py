...
def main() -> int:
    ...
    for concept in concepts:
        ...
        if concept.get("id") == "stability-under-replanning":
            for marker in ["1.5 meters left", "E = distance_error^2 + 0.5*sideways_speed^2", "E = 1.5^2 + 0.5*0.8^2 = 2.57", "E = 0.9^2 + 0.5*0.5^2 = 0.935", "0.5 meters left with sideways speed 0.3 m/s right", "E = 0.5^2 + 0.5*0.3^2 = 0.295", "burden keeps falling", "1.0 meters right", "sideways speed 0.6 m/s left", "E = 1.0^2 + 0.5*0.6^2 = 1.18", "burden rose from 0.935 to 1.18", "terminal cost, terminal set, or a decrease condition", "x_{k+1}=f_closed(x_k)", "V(f_closed(x)) - V(x) &lt;= -stage_cost(x,pi_MPC(x))", "delta V = 0.935 - 2.57 = -1.635", "delta V = 0.295 - 0.935 = -0.640", "delta V = 1.18 - 0.935 = +0.245", "violates a nonincrease test", "stage cost for the second state were 0.10", "delta V &lt;= -0.10", "accept -0.640 and reject +0.245", "Feasibility alone would only say the next optimization exists", "terminal set gives the final predicted state", "cycling, drifting, or amplifying velocity"]:
                if marker not in text:
                    errors.append(f"stability under replanning concept page missing decrease marker: {marker}")
            if len(words) < 980:
                errors.append(f"stability under replanning concept page below core richness floor: {len(words)} < 980")
